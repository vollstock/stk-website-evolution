const detectPlatform = () => {
    ...
    return /iphone|ipad|ipod|ios/.test(platformHint) ? 'iOS' : /android/.test(platformHint) ? 'Android' : /windows|win32|win64/.test(platformHint) ? 'Windows' : /macintosh|mac os x|mac_powerpc|darwin/.test(platformHint) ? 'macOS' : /nintendo|switch|nx/.test(platformHint) ? 'Nintendo Switch' : /linux/.test(platformHint) ? 'Linux' : 'Other';
};
const detectArchitecture = () => {
    ...
    return /aarch64|arm64/.test(architectureHint) ? 'ARM 64-bit' : /armv7|armv7l/.test(architectureHint) ? 'ARM 32-bit' : /x86_64|x64|amd64/.test(architectureHint) ? 'Intel / AMD 64-bit' : /i686|i386|x86/.test(architectureHint) ? 'Intel / AMD 32-bit' : /riscv64/.test(architectureHint) ? 'RISC-V 64-bit' : '';
};
const chooseDownload = (box) => {
    ...
    const selected = platformOptions.find((option) => option.dataset.architecture === architecture) || platformOptions[0] || options[0];
    if (!selected) return;
