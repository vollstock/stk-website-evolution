...
<footer class="w-full bg-gray-950 p-3 lg:px-6 lg:py-8">
        ...
        <?php foreach ($site->menu()->toStructure() as $i => $item): ?>
            <div class="text-sm text-gray-600">
                <?php if ($item->hasSubmenu()->toBool()): ?>
                    <h3 class="font-semibold text-gray-400 mb-6"><?= $item->title() ?></h3>
                    <ul>

                        <?php foreach ($item->subMenu()->toStructure() as $child): ?>
                            <li class="my-2">
                                <a href="<?= $child->link()->toUrl() ?>" class="flex gap-3 group">
                                    <span class="group-hover:text-amber-600"><?= $child->title() ?></span>
                ...
                <?php else: ?>
                    <a href="<?= $item->link()->toUrl() ?>" class="font-semibold text-gray-400">
                        <?= $item->title() ?>
                    </a>
                <?php endif ?>
            </div>
